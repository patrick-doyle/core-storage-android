...
class CoreStorageProvider internal constructor(
    private val context: Context,
    private val logger: CoreStorageLogger,
    private val memoryCache: MemoryCache = DefaultMemoryCache(),
) {
    ...
    companion object {
        ...
        private var instance: CoreStorageProvider? = null

        fun get(context: Context): CoreStorageProvider {
            return get(context, CoreStorageAndroidLogger)
        }

        fun get(context: Context, logger: CoreStorageLogger): CoreStorageProvider {
            return instance ?: synchronized(this) {
                CoreStorageProvider(context.applicationContext, logger).also { instance = it }
            }
        }
    }
}
